...
// Cap nhat
void ChangeMeaning(char* FileName,WORD word)
{
    FILE *file = fopen(FileName, "r+b");
    WORD currentWord;
    while (fread(&currentWord, sizeof(currentWord), 1, file) == 1) {
        if (strcmp(currentWord.Name, word.Name) == 0) {
            fseek(file, -sizeof(currentWord), SEEK_CUR);
